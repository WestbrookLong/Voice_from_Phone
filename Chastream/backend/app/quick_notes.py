...
def process_quick_note(audio_path: Path, style: str) -> dict:
    ...
    result = organize_note(transcript, style)
    result["rawTranscript"] = transcript
    result["transcription"] = raw
    return result


def organize_note(transcript: str, style: str) -> dict:
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "content 是去除口头禅和重复表达后的完整书面正文。"
                    ),
                },
                {
                    "role": "user",
                    "content": f"整理风格：{style}\n\n原始转写：\n{transcript}",
                },
            ],
        },
        timeout=120,
